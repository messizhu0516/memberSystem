package com.zhuqifeng.commons.shiro.captcha;
...
import com.zhuqifeng.commons.utils.base.WebUtils;
import com.zhuqifeng.commons.utils.sequence.SequenceUtil;

public class FlySkyCaptcha implements InitializingBean {
	private final static Logger logger = LogManager.getLogger(FlySkyCaptcha.class);
	private static final String DEFAULT_COOKIE_NAME = "flySky-captcha";
	private final static String DEFAULT_CHACHE_NAME = "flySkyCaptchaCache";
	...
	private Cache<String, String> flySkyCaptchaCache;

	public FlySkyCaptcha() {
		this.cacheName = DEFAULT_CHACHE_NAME;
		this.cookieName = DEFAULT_COOKIE_NAME;
	}

	public FlySkyCaptcha(CacheManager cacheManager) {
		this();
	...
	public void afterPropertiesSet() throws Exception {
		...
		this.flySkyCaptchaCache = cacheManager.getCache(cacheName);
	}
	...
	public void generate(HttpServletRequest request, HttpServletResponse response) {
		...
		if (StringUtils.isBlank(cookieValue)) {
			hasCookie = false;
			cookieValue = SequenceUtil.getUUID();
		}
		...
		flySkyCaptchaCache.put(cookieValue, captchaCode);
	}
	...
	public boolean validate(HttpServletRequest request, HttpServletResponse response, String userInputCaptcha) {
		...
		String captchaCode = flySkyCaptchaCache.get(cookieValue);
		...
		if (result) {
			flySkyCaptchaCache.remove(cookieValue);
